...
?>
<form action="scripts/sale/add.php" method="POST" class="text-white flex flex-col h-full gap-4 px-2 pb-8" enctype="multipart/form-data">
    ...
    <dl class="">
        <div class="grid grid-cols-2 gap-4">
            <div class="">
                <dt class="text-sm font-medium leading-6 text-gray-300">Nazwa sali</dt>
                    <input type="hidden" name="school_id" value="<?=$school_id?>">
                    <input name="sale" required type="text" value="" placeholder="np. 13" class="w-full text-gray-400 text-sm focus:text-white bg-[#0e0e0e] focus:ring-0 focus:outline-0 border-b border-white/10 py-2 theme-border-focus duration-150"></input>
            </div>
            <div class="col-span-1">
                <dt class="text-sm font-medium leading-6 text-gray-300">Budynek</dt>
                    <select name="build_id" class="mt-1 outline-none duration-150 capitalize relative w-full cursor-default rounded-md bg-[#0e0e0e] focus:text-white py-1.5 pl-3 pr-10 text-left text-gray-400 shadow-sm ring-1 ring-inset ring-[#3d3d3d] focus:outline-none focus:ring-2 theme-ring-focus sm:text-sm sm:leading-6" required>
                            <option value="" class="hidden" disabled selected>Wybierz</option>
                            <?php
                            $sql = "SELECT * FROM `buildings` where school_id = $school_id;";
                            $result = mysqli_query($conn, $sql);
                            while($row = mysqli_fetch_assoc($result)) {
                                echo '<option value="'.$row['build_id'].'" class="capitalize">'.$row['name'].' - '.$row['street'].'</option>';
                            }
                            ?>
                    </select>
            </div>
